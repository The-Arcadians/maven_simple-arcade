...
public class NumberGuessGame implements GameInterface {
    ...
    int correctGuess = guessEngine.randomNumber();
        ...
        while(play){
            ...
            while(true){
                ...
                if(input == correctGuess){
                    ...
                    break;
                } else if (correctGuess > input){
                    guessEngine.wrongMessageGen();
                    console.println("Number is higher.");
                    promptHint();
                } else if (correctGuess < input){
                    guessEngine.wrongMessageGen();
                    console.println("Number is lower.");
                    promptHint();
                }
            }
            guessEngine.promptContinue(input);
    ...
    /**
     * Created a method so that it can be called anywhere to prompt a hint
     * Logic credit: Leon 100% beast
     */
    public void promptHint() {
        ...
        for (int minNumberToGuess = 1; minNumberToGuess < maxValue; minNumberToGuess += incrementValue) {
            ...
            if (correctGuess > minNumberToGuess && correctGuess <= maxNumberToGuess) {
                String message = "\nHint: The number is somewhere between %s-%s";
                console.println(message, minNumberToGuess, maxNumberToGuess);
            }
        }
    }
    ...
    /**
     * Moved below commented out code to NumberGuessEngine. Created methods that randomize the messages.
     * Will review changes with team before removing.
     */
...
//    private IOConsole wrongAnswerConsole = new IOConsole(AnsiColor.RED);
    ...
    /**
     *Commented out run() method for now. Will review changes and check with team before removing.
     */
//    @Override
//    public void run() {
//        boolean play;
